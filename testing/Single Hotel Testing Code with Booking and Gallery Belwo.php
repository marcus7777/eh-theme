...
if ( $children == 0 AND $adults < 3 ) {
  ...
  if ( is_object( $obj ) ) {
    ...
    // SECOUND OUTPUT HTML STRING FOR THE CROSSTAB BELOW DETAILING THE COSTS PER NIGHT

   

    $tble2 .= '<div class="pernight">
    ...
    foreach ( $obj->lr_rates->hotel->hotel_rooms->room as $rm ) {
      ...
      $tble2 .= '<div class="coldes">';

      $tble2 .= '<div class="rmdes2">';

      $tble2 .= (string) $rm->type_description;

      $tble2 .= '</div>';

      foreach ( $rm->rate as $rt ) {

        $tble2 .= '<div class="price2">' . (string) $rt->price . '</div>';
