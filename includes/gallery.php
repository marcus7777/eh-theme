...
if ( !empty($attachments) ) :
	...
	foreach ( $attachments as $att_id => $attachment ) {
		...
		if ( get_option('woo_resize') == "true" && $use_timthumb == "true" )
  			...
  			$photo_output .= '<div' . $style . '><a href="'. $src[0] .'" rel="lightbox-group" class="thickbox" title="'.$attachment->post_excerpt.'"><img src="'. $src[0] .'" width="100%" class="single-photo" alt="'.$attachment->post_excerpt.'" /></a>'.$caption.'</div>'; 
