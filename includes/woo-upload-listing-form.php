...
?>
	<form name="woo_listings_upload_listing" method="post" action="">
<?php

	// Once in, display a form using dynamic data from the database.
	
	if ( isset( $_POST['woo_post_type'] ) && isset( $_POST['woo-action'] ) ) {
		...
		$button_text = __( 'Upload Your Listing' , 'woothemes' );
		$action = 'upload';
		...
		if ( $_is_processed ) {
			...
			$message = sprintf( __( 'Your %s has been submitted for review.', 'woothemes' ), $post_type );
...
?></form><?php
	...
	echo '<p class="woo-sc-box info">' . __( 'Please login to post a listing', 'woothemes' ) . '</p>';
